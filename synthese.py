...
# Load source image
source = Image.open("data/text3.png")
source_array = np.array(source)
...
# Setup the figure and axis for real-time plotting
fig, ax = plt.subplots()
img_display = ax.imshow(destination_array, cmap='gray', interpolation='none')
ax.axis('off')

def update(frame):
    global binary_array, edges
    process_pixel()
    binary_array = np.any(destination_array != [0, 0, 0], axis=-1).astype(int)
    edges = utils.find_unfilled_with_neighbors(binary_array)
    
    img_display.set_data(destination_array)
...
# Create animation
ani = FuncAnimation(fig, update, interval=0.001, blit=True)

plt.show()

# Save the final result
output_image = Image.fromarray(destination_array)
file_name = f"output_image_{width}x{height}_p{patch_size}_{np.random.randint(1000, 9999)}.png"
output_image.save(file_name)
